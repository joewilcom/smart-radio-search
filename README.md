...
5.  **Chat Recommendations (Backend `/chat` in `app.py`):**
    ...
    * The backend prepends a system prompt so OpenAI suggests useful search tags in double quotes. Each tag is a single word and becomes a clickable suggestion. It may also recommend station names when possible.
